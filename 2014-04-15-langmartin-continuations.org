...
Composed by the interpreter and opaque to the program.

** Closures Capture Part of the Stack

The call stack provides the context to execute code arranged in
procedures, so it is the mechanism for assigning local variable
binding storage.
...
address, so (in node, e.g.) we don't return from a procedure. We
write our procedures in the form of callbacks.

Each line of code is encapsulated in it's own anonymous procedure
which captures the local variable scope and expects a standard async
parameter interface.
...
can prompt the user, say, and then invoke the procedure to retry the
step with the same parameters.
...
+ Tagged, delimited continuations
